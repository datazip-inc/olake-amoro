...
// writes task-specific logs to separate files organized by process ID
public class TaskLogger implements AutoCloseable, Serializable {
  ...
  public TaskLogger(long processId, int taskId, String tableName) {
    this.processId = processId;
    this.taskId = taskId;
    // why do we need tableName, can we please remove it
  ...
  private synchronized void ensureWriter() throws IOException {
    if (writer == null) {
      ...
      logFile = processDir.resolve(taskId + ".log");
  ...
  private synchronized void log(String level, String message, Throwable throwable) {
    try {
      ...
      System.err.println("Failed to write to task log: " + e.getMessage());
      e.printStackTrace();
    }
  }
